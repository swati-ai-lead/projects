...
function selectedRentMonth() { return `${document.querySelector("#rent-month").value || monthKey.slice(0, 7)}-01`; }
...
function renderHistory() {
  ...
  const months = [...new Set([...state.rentHistory, ...state.utilityHistory].map(item => item.month))].sort().reverse();
  if (!months.includes(monthKey)) months.unshift(monthKey);
  const selected = select.value || months[0] || monthKey;
  select.innerHTML = months.map(month => `<option value="${month}">${new Intl.DateTimeFormat("en-US", { month: "long", year: "numeric", timeZone: "UTC" }).format(new Date(`${month}T00:00:00Z`))}</option>`).join("");
...
function renderTenants() { const peco = state.utilities.find(item => item.service === "PECO"); const water = state.utilities.find(item => item.service === "Water"); document.querySelector("#tenant-table").innerHTML = state.tenants.map(item => `<tr><td><strong>${item.unit_name}</strong></td><td>${item.full_name}</td><td>${item.email || "-"}<br><small>${item.phone || ""}</small></td><td>${item.lease_start} to ${item.lease_end}</td><td>${money(item.monthly_rent)}</td><td>${peco ? `${money(Number(peco.amount) / 4)}<br><small>PECO bill / 4</small>` : "-"}</td><td>${water ? `${money(Number(water.amount) / 4)}<br><small>Water bill / 4</small>` : "-"}</td><td><span class="status ${item.status === "Active" ? "paid" : ""}">${item.status}</span></td><td><div class="tenant-actions">${item.lease_url ? `<a class="text-link" href="${item.lease_url}" target="_blank" rel="noreferrer">View lease</a>` : ""}${isAdmin ? `<button class="small-button" data-edit-tenant="${item.id}">Edit</button>${item.phone ? `<button class="small-button" data-copy-reminder="${item.id}">Copy reminder</button>` : ""}` : ""}</div></td></tr>`).join("") || "<tr><td colspan='9'>No tenants have been added yet.</td></tr>"; }
function renderAll() {
...
document.querySelector("#rent-month").addEventListener("change", renderRent);
...
document.querySelector("#entry-form").addEventListener("submit", async event => {
  ...
  if (type === "rentHistory" || type === "utilityHistory") {
    const id = form.dataset.id;
    const table = type === "rentHistory" ? "rent_history" : "utility_history";
    const historicalUtility = type === "utilityHistory" ? state.utilityHistory.find(item => item.id === id) : null;
    const updates = type === "rentHistory" ? { rent: Number(data.get("rent")) } : { amount: Number(data.get("amount")), due: monthEndDue(historicalUtility.month) };
